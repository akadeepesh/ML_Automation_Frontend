    ...
    {type ? (
      <>
        ...
        <div className="flex ml-1">
          ...
          <div
            style={{
              width: `${divWidth + 15}px`,
              height: `${divHeight * 25 + 30}px`,
            }}
            className="bg-[#2A3942] text-white pl-2 mt-1 pr-2 rounded-tl-none rounded-2xl break-words"
          >
            <span style={{ width: 255 }} className="text-lg w-4 h-4">
              {message}
            </span>
          </div>
        </div>
      ...
      <>
        ...
        <div className="flex mt-1 mr-1 ml-auto">
          <div
            ...
            className="bg-[#2A3942] text-white flex-initial pl-2 mt-1 pr-2 rounded-tr-none rounded-2xl break-words"
          >
            <span className="text-lg">{message}</span>
          </div>
          <div className="logo mr-1 ml-1">
            <span style={generateAvatarColor(name)}>
              {name
                .split(" ")
...
const Home = () => {
  ...
  const [botResponse, setBotResponse] = useState(
    "I am currently in development mode, thanks for your patience"
  );
  ...
  return (
    <div
      className={`fixed flex right-4 bottom-[4%] ${
        isChatOpen ? "z-40" : "z-20"
      }`}
    >
      <div
        className={`w-[var(--w288)] h-[var(--h384)] chatBox ${
          isChatOpen ? "flex animate-show z-10" : "hidden-chat"
        } rounded-xl bg-blackish`}
      >
        <span
          className={`bg-blackish text-blueish absolute w-fit h-fit cursor-pointer rounded-full p-3`}
          onClick={handleClick}
        >
        ...
        <div
          style={{
            width: "calc(var(--w288) - 23px)",
            height: "calc(var(--h384) - 23px)",
          }}
          className="m-auto chatarea rounded-xl flex flex-col"
        >
          <div
            ref={chatContainerRef}
            className="mt-14 scroll-smooth mb-2 h-full overflow-y-auto"
          >
          ...
          <div className="mt-auto">
            <div className="flex  bg-[#2A3942] rounded-b-xl">
              <div className="ml-2 w-full">
                <input
                  className="bg-[#202C33] text-white indent-2.5 rounded-xl p-1 w-full h-[80%] focus:outline-none my-2"
                  value={input}
              ...
              <span
                className="text-[#8696A0] cursor-pointer rounded-full ml-2 w-fit h-fit p-1 mt-3"
                type="submit"
      ...
      <span
        className={`bg-blackish text-blueish mt-auto openit cursor-pointer rounded-full flex p-1 ${
          isChatOpen ? "openitRad" : "ani"
        }`}
        onClick={handleClick}
      >
        {icon === "ripple" ? (
