  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8000/api/user/stemlem/",
        { sentence: sentence, method: method }
      );
      console.log(response);
      if (response.data.shorted_words === undefined) {
        console.error("Error fetching sentence");
      } else {
        setFilteredSentence(response.data.shorted_words);
      }
    } catch (error) {
      console.error(error);
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <div className="w-1/2 bg-white rounded shadow p-6 m-4">
        <div className="flex justify-between mb-5">
          <h1 className="mb-4 text-xl font-semibold text-gray-700">Tokenize</h1>
          <div>
            <Select
              label="Select Method"
              value={method}
            ...
            >
              <Option value="stemming" defaultChecked>
                Stemming
              </Option>
              <Option value="lemmatize">Lemmatization</Option>
            </Select>
          </div>
        </div>
